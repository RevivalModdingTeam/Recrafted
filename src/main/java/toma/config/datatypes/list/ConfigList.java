...
public class ConfigList<T> extends TypeImpl<List<T>> {
    ...
    public static class ListWidget<T> extends ConfigWidget<List<T>> {
        ...
        public ListWidget(int x, int y, int w, int h, ConfigList<T> list, Screen parent) {
            super(x, y, w, h, list);
            this.parent = parent;
        }
        ...
        public boolean mouseClicked(double mouseX, double mouseY, int button) {
            if(isHovered) {
                Minecraft.getInstance().displayGuiScreen(new ModConfigListScreen<>((ConfigList<T>) this.type, parent));
                return true;
            }
            return false;
        }
